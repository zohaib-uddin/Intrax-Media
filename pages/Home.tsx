...
import { Check, Star, Play, X, ChevronDown, ChevronUp, Clock, Video, Globe, ArrowRight, Zap, Target, Shield, Award, User, Settings, Briefcase, Layers, BarChart, Users, ShoppingCart, TrendingUp } from 'lucide-react';
...
export const Home: React.FC = () => {
 ...
 const strategies = [
  ...
  Scale Your <span className="text-brand-500 yellow-glow">Brand.</span>
</h1>
...
{/* <h1 className="text-6xl md:text-8xl font-display font-black uppercase leading-[0.9] tracking-tighter mb-8 text-black">
  ...
  {[
    { text: "We manage complete Marketing Operations (MOM) to streamline your growth engine.", icon: Settings },
    { text: "Professional marketing support for both B2B and DTC brands.", icon: Briefcase },
    { text: "Strategic integration of paid and organic channels for scalable results.", icon: Layers },
    { text: "Connect with the right audience and increase sales through data-driven campaigns.", icon: BarChart }
  ].map((item, i) => (
    <div key={i} className="bg-white p-8 rounded-none border border-brand-500 flex flex-col items-start text-left hover:-translate-y-2 transition-all duration-300 shadow-xl group hover:shadow-brand-500/10 overflow-hidden relative">
      <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
        <item.icon size={80} className="text-brand-500 transform rotate-12" />
      </div>
      <div className="mb-4 w-12 h-12 rounded-full flex items-center justify-center transition-all group-hover:bg-brand-500 group-hover:scale-110">
         <item.icon className="text-brand-500 group-hover:text-black transition-colors" size={28} strokeWidth={3} />
      </div>
      <p className="text-sm font-medium text-black leading-snug group-hover:text-brand-500 transition-colors relative z-10">{item.text}</p>
    </div>
  ))}
</div>
        ...
        <FadeInSection key={idx} delay={idx * 50} className="group relative bg-white rounded-3xl p-8 md:p-10 border border-brand-100 hover:border-brand-500 transition-all duration-300 hover:shadow-2xl flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="flex-grow">
            <div className="flex items-center gap-4 mb-4">
              <h3 className="text-2xl md:text-3xl font-black text-black group-hover:text-brand-500 transition-colors">{strat.title}</h3>
              <strat.icon className="text-brand-500 group-hover:scale-125 transition-transform shrink-0" size={32} />
            </div>
